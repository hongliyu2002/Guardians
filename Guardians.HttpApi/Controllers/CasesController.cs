...
public sealed class CasesController : ControllerBase
{
    ...
    {
        var result = await _caseAppService.GetCaseAsync(caseID).ConfigureAwait(false);
        return StatusCode(result.Code, result);
    }
    ...
    {
        var result = await _caseAppService.ListPagedCasesAsync(reporterNo, startDate, endDate, pageNo, pageSize).ConfigureAwait(false);
        return StatusCode(result.Code, result);
    }
}